...
function updateTrafficFromSEMRush() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Data");
  var domains = sheet.getDataRange().getValues().splice(1);
  var lastDate = sheet.getRange('Z1').getValue().toString();
  var updatedDate = '';
  var updatedValues = [];
  ...
  for (var i = 0; i < domains.length; i++) {
    ...
    if (domains[i] != '') {
      ...
      var parsableData = response.getContentText();
      if (parsableData.indexOf('ERROR') == -1) {
        var value = parsableData.split('\n')[1];
        if (value.indexOf(';') > -1) {
          updatedDate = value.split(';')[0];
          var updatedValue = value.split(';')[1];
          if (updatedDate != lastDate) {
            updatedValues.push(updatedValue);
          } else return false;
        }
      } else {
        updatedValues.push('No Data');
        continue;
      }
    }
  }
  //convert array to object[][] 
  updatedValues.unshift(updatedDate);
  var array = updatedValues.map(function(el) {
    return [el]
  });
  sheet.insertColumns(27);
  sheet.getRange(1, 27, domains.length+1, 1).setValues(array);
